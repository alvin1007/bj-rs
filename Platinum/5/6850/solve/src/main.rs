fn main() {
    ...
    for _ in 0..n {
        buffer.clear();
        std::io::stdin().read_line(&mut buffer).unwrap();
        let p = buffer.split_whitespace().map(|x|x.parse::<i64>().unwrap()).collect::<Vec<i64>>();
        points.push((p[0], p[1]));
    }

    let mut res = convex_hull(&mut points);
    ...
    for i in 0..res.len()-1 {
        space += res[i].0*res[i+1].1 - res[i].1*res[i+1].0
    }
    ...
    writeln!(stdout, "{}", (space.abs() as f64 / 100.) as i64).unwrap();
}
