fn main() {
    ...
    let mut n = buf.trim().parse::<u128>().unwrap();
    while n % 4 == 0 { n /= 4; }
    if n % 8 == 7 {
        println!("4");
        return;
    ...
    let mut s: std::collections::HashSet<u128> = std::collections::HashSet::new();
    ...
    if (n as f64).sqrt() as u128 * (n as f64).sqrt() as u128 != n {
        println!("2");
    } else {
        println!("1");
    }
