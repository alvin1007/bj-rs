fn main() {
    ...
    let a = BigInt::new(buffer.trim());

    buffer.clear();
    std::io::stdin().read_line(&mut buffer).unwrap();

    let b = BigInt::new(buffer.trim());

    let add = a.add(&b);
    let sub = a.subtract(&b);
    let mul = a.multiply(&b);

    if add.sign {
        for s in add.digits.iter().rev() {
            print!("{}", *s)
        }
        print!("\n")
    } else {
    ...
    if mul.digits.len() == 1 && mul.digits[0] == 0 {
        print!("0")
    } else {
        if mul.sign {
            for s in mul.digits.iter().rev() {
                print!("{}", *s)
            }
        } else {
            print!("-");
