fn main() {
    ...
    let t = buf.split_ascii_whitespace().collect::<Vec<&str>>();
    let a = BigInt::new(t[0]);
    let b = BigInt::new(t[1]);
    let mut c = a.add(&b);
    c.digits.reverse();
    println!("{}", c.digits.iter().map(|&x| x.to_string()).collect::<Vec<String>>().join(""));
}
